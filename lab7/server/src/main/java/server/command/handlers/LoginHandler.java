...
public final class LoginHandler implements ServerCommand {
    ...
    public Response execute(Request request, ServerContext context) {
        ...
        if (context.storage().checkUser(creds.getLogin(), hash)) {
            return Response.ok(request.getRequestId(), "Вход выполнен.");
        }
        return Response.error(request.getRequestId(), "Неверный логин или пароль.");
    }
}
