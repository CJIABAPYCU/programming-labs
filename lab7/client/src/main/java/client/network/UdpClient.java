...
public final class UdpClient implements AutoCloseable {
    ...
    /**
     * Отправляет запрос и ждёт подходящий ответ. Запросы и ответы делятся
     * на прикладные фрагменты, поэтому не ограничены одной UDP-датаграммой.
     *
     * @param request запрос
     * @return ответ с тем же {@code requestId}
     * @throws ServerUnavailableException если все попытки завершились таймаутом
     * @throws IOException при сетевой ошибке или ошибке сериализации
     * @throws ClassNotFoundException если класс ответа недоступен
     */
